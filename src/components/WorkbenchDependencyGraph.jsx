...
const resolveCardAnchor = (rect, side, offset = 4) => {
...
const buildConnectorPath = (start, end, orientation) => {
  if (!start || !end) return '';
  if (orientation === 'horizontal') {
...
const MiniMapPreview = ({ layout, anchorMetaById = {} }) => {
  if (!layout?.transform) {
    return (
...
const WorkbenchDependencyGraph = ({
  ...
  const resolveNodePosition = React.useCallback(
    (nodeId) => nodePositions[nodeId],
    [nodePositions]
  );

  const getCardHeight = React.useCallback(
    (nodeId) => (expandedNodeIds.has(nodeId) ? GRAPH_CARD_EXPANDED_HEIGHT : GRAPH_CARD_COLLAPSED_HEIGHT),
    [expandedNodeIds]
  );

  const getCardRect = React.useCallback((nodeId) => {
  ...
  const resolveAnchor = React.useCallback((nodeId, internalNodeId, rect, side) => {
    if (!rect) return null;
  ...
  return (
    ...
    >
      ...
      <div className="absolute right-4 bottom-4 z-30 flex flex-col gap-1 rounded-lg border border-gray-200 bg-white/90 p-1 shadow-sm dark:border-slate-700 dark:bg-slate-900/90">
        <button
